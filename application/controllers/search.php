...
Class Search extends CI_Controller
{
    ...
    public function search_process()
    {
        ...
        if (isset($_SESSION['logged_in'])) {
            ...
            $condition = $_POST['input'];
            
            $SQLquery = "
            (SELECT * FROM `cafe` WHERE cname like '$condition') 

            UNION (SELECT * FROM `cafe` WHERE cname like '%$condition%') 
            UNION (SELECT * FROM `cafe` WHERE cname like '$condition%') 
            UNION (SELECT * FROM `cafe` WHERE cname like '%$condition')
            ";
            ...
            $SQLquery2 = "
            (SELECT * FROM `cafe` WHERE carea like '$condition') 

            UNION (SELECT * FROM `cafe` WHERE carea like '%$condition%') 
            UNION (SELECT * FROM `cafe` WHERE carea like '$condition%') 
            UNION (SELECT * FROM `cafe` WHERE carea like '%$condition')
            ";
            $queryarea = $this->db->query($SQLquery2);

                $SQLquery3 = "
                (SELECT * FROM `cafe` WHERE cicecream = 1 ) 
                ";
                $queryicecream = $this->db->query($SQLquery3);         
            ...
            $SQLquery3   = "
            (SELECT * FROM `users`) ";
            $querycafeid = $this->db->query($SQLquery3);
        ...
        foreach ($sumoftop as $key => $value) {
            ...
            */
            ...
            if ($countnull > 22) { #if not rate > 8 show top
                ...
                $data["arealist"]    = $queryarea->result();            
                $data["icecreamlist"]   = $queryicecream->result();     
            ...
            $this->load->view("searchresult_page", $data);
            $this->load->view("footer_in", $data);
                ...
                for ($i = 0; $i < 30; $i++) {
                    ...
                    for ($j = 0; $j < 30; $j++) {
                        ...
                        if ($queryContentbase->num_rows() > 0) {
                            ...
                            $multiplyof2rows = ($checkint * $checkintJ + $checkint2 * $checkintJ2 + $checkint3 * $checkintJ3 + $checkint4 * $checkintJ4 + $checkint5 * $checkintJ5 + $checkint6 * $checkintJ6 + $checkint7 * $checkintJ7 + $checkint8 * $checkintJ8 + $checkint9 * $checkintJ9);
                            ...
                            $similarity = ($multiplyof2rows / (sqrt($checkint + $checkint2 + $checkint3 + $checkint4 + $checkint5 + $checkint6 + $checkint7 + $checkint8 + $checkint9) * (sqrt($checkintJ + $checkintJ2 + $checkintJ3 + $checkintJ4 + $checkintJ5 + $checkintJ6 + $checkintJ7 + $checkintJ8 + $checkintJ9))));
                ...
                if ($query->num_rows() > 1) {
                    for ($i = 0; $i < $query->num_rows(); $i++) {
                        ...
                        $this->db->join('rating_cafe', 'rating_cafe.cid=cafe.cid', 'left');
                        $this->db->join('users', 'users.uid=rating_cafe.uid', 'left');
                        $this->db->where('cafe.cid =' . $result[$i]->cid);
                        $query2 = $this->db->get();
                        ...
                        if ($query2->num_rows() > 1) {
                            ...
                            for ($i = 0; $i < $query2->num_rows(); $i++) {
                                ...
                                $totalrateArray[] = $result2[$i]->ratescore;
                                $userArray[]      = 1;
                            }
                        }
                ...
                (
                    ...
                    WHERE dessert1.dname like '$condition'
                )
                UNION 
                (    
                    SELECT dessert1.*, cate.catename
        ...
        ";
            ...
            $data["arealist"]        = $queryarea->result();
            $data["icecreamlist"]   = $queryicecream->result();
            $data["condition"] = $condition = $_POST['input'];
            $data['recommendedcafe'] = $collecttop;
            $data["sumof"] = $sumof;
            $data['recommendedrating'] = $topratingcafevalue;
            ...
            // $condition = "catename =" . "'" . $_POST['input'] . "'";
            // $this->db->select('*');
            // // $this->db->select('dessert.dname');
            // // $this->db->select('categories.catename');
            // // $this->db->select('rating_dessert.ratedscore');
            // $this->db->from('dessert_categories');
            // $this->db->join('dessert','dessert.did=dessert_categories.dcdid','inner');
            // $this->db->join('rating_dessert','rating_dessert.did=dessert.did','inner');
            // $this->db->join('categories','categories.cateid=dessert_categories.dccid','inฟner');
            ...
            // $result = $this->db->query($sqlquery, array( $data['catename']));
        }
    ...
    public function detail($id)
    {
        ...
        if (isset($_SESSION['logged_in'])) {
            ...
            if ($user != 0) {
                ...
                $data['session_username'] = $session_data['username'];
                $data["detaildata"]       = $itemdata;
                $data["totalscore"]       = round($totalrate / 1, 2);
                $this->load->view("detail_page", $data);
            }
        } else {
            if ($user != 0) {
        ...
        // $condition = "dessert.did =" . "'" . $id . "'";
        // $this->db->select('*');
        // $this->db->from('dessert_categories');
        // $this->db->join('dessert','dessert.did=dessert_categories.dcdid','inner');
        // $this->db->join('rating_dessert','rating_dessert.did=dessert.did','inner');
        // $this->db->join('categories','categories.cateid=dessert_categories.dccid','inner');
        // $this->db->where($condition);
        // $query = $this->db->get();
        ...
        //         $this->load->view("detail_page_tmp", $data);
